...
import (
	...
	"net/http"
	response "product-app/controller/reponse"
...
func (productController *ProductController) AddProduct(c echo.Context) error {
	...
	bindErr := c.Bind(&addProductRequest) //requestbody'deki degerleri direkt olarak buraya yerlestirecek --> Bind()
	if bindErr != nil {
...
func (productController *ProductController) UpdatePrice(c echo.Context) error {
	...
	if err != nil {
		return c.JSON(http.StatusBadRequest, response.ErrorResponse{ // kullanıcını controller kısmında yaptıgı hata -> BadReguest
